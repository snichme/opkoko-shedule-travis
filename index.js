...
const renderAgenda = (el, program, currentDay) => {
    const now = nowAsTime();
    const currentRunning = currentForDay(program, currentDay, now);        
    renderToDom(<Agenda onPresSelected={showInfoModal} {...currentRunning} />, el);
}
...
const run = () => {
  when.all([getSchedule('schedule.json'), getFavourites(), getActiveTab()]).then(function(res) {
    ...
    let programWithFavs = program;
    if(favs.length > 0) {
        let personal = createPersonalSchedule(program, favs);
        programWithFavs  = program.concat(personal);
    }
    
    _.each(document.querySelectorAll('[opkoko]'), (el) => {
        console.log();
        var comp = el.getAttribute("opkoko");
        if(comp === "current") {
            renderCurrent(el, program, 0);
            return;
        } 
        if(comp === "program") {
            renderProgram(el, programWithFavs, tab);
            return;
        }
        if(comp === "agenda") {
            renderAgenda(el, program, 0);
            return;
        }
